...
void test_signal_calculation() {
    ...
    float signal = calculate_signal_strength(0.1, 2.4);
    printf("Signal at 0 km: %.2f\n", signal);
    
    signal = calculate_signal_strength(1.0, 3.5);
...
int main() {
    ...
    for (int i = 0; i < MAX_DEVICES; i++) {
        MobileDevice *device_ptr = &devices[i];
        printf(device_ptr->technology);
